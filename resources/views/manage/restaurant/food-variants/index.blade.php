...
@section('title', 'Food Variants - Zomato Partner')
...
<div class="nxl-content">
    ...
    <div class="page-header">
        ...
        <div class="page-header-right ms-auto d-flex align-items-center gap-2">
            <a href="{{ route('restaurant.food-variants.create') }}" class="btn btn-danger text-white fw-semibold" style="background-color: #cb202d; border: none;">
                <i class="feather-plus me-1"></i> Add Variant
            </a>
        </div>
    </div>
    <!-- [ page-header ] end -->
    ...
    <div class="main-content">
        ...
        <div class="card stretch stretch-full mb-4 border-0 shadow-sm rounded-3">
            <div class="card-body p-3">
                <form action="{{ route('restaurant.food-variants.index') }}" method="GET" class="row g-2 align-items-center">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-3">
                        <select name="food_id" class="form-select">
                            <option value="">All My Food Items</option>
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        ...
                        <a href="{{ route('restaurant.food-variants.index') }}" class="btn btn-light border text-secondary fw-semibold">Reset</a>
                        <a href="{{ route('restaurant.food-variants.index', ['trashed' => 1]) }}" class="btn btn-outline-danger fw-semibold" title="View Trashed Variants">
        ...
        <div class="card stretch stretch-full border-0 shadow-sm rounded-3">
            <div class="card-header border-bottom py-3">
                <h5 class="card-title mb-0 fw-bold">Variants for {{ $restaurant->restaurant_name }}</h5>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($variant->sale_price)
                                            ...
                                            <small class="text-success text-muted text-decoration-line-through">
                                    ...
                                    <td>
                                        @if(!$variant->trashed())
                                            <form action="{{ route('restaurant.food-variants.toggle-status', $variant->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="d-flex align-items-center justify-content-end gap-1">
                                            @if(!$variant->trashed())
                                                <a href="{{ route('restaurant.food-variants.show', $variant->id) }}" class="btn btn-sm btn-light border text-info p-2 rounded-2" data-bs-toggle="tooltip" title="View Details">
                                                    <i class="feather-eye"></i>
                                                </a>
                                                <a href="{{ route('restaurant.food-variants.edit', $variant->id) }}" class="btn btn-sm btn-light border text-primary p-2 rounded-2" data-bs-toggle="tooltip" title="Edit">
                                                    <i class="feather-edit"></i>
                                                </a>
                                                <form action="{{ route('restaurant.food-variants.destroy', $variant->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to soft delete this variant?');">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @else
                                                <form action="{{ route('restaurant.food-variants.restore', $variant->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    <button type="submit" class="btn btn-sm btn-success p-2 rounded-2" title="Restore">
                                                        <i class="feather-rotate-ccw"></i> Restore
                                                    </button>
                                                </form>
                                                <form action="{{ route('restaurant.food-variants.force-delete', $variant->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to permanently delete this variant?');">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-5 text-muted">
                                        ...
                                        No food variants found for your menu.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
