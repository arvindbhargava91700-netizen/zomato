...
@section('title', 'Withdrawals & Payouts - Restaurant Partner')
...
<div class="page-header d-flex align-items-center justify-content-between mb-4">
    <div class="page-header-left d-flex align-items-center">
        <div class="page-header-title">
            <h5 class="m-b-10 fw-bold">Restaurant Payouts & Withdrawals</h5>
        </div>
        <ul class="breadcrumb">
    ...
    <div class="page-header-right d-flex gap-2">
        <a href="{{ route('restaurant.withdrawals.create') }}" class="btn btn-primary shadow-sm px-3">
            <i class="feather-plus-circle me-1"></i>Request Payout
        </a>
    </div>
</div>
<!-- [ page-header ] end -->
...
<div class="main-content">
            ...
            <div class="card stretch stretch-full border-0 shadow-sm rounded-4 metric-card bg-white">
                <div class="card-body p-3 d-flex align-items-center gap-3">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <div class="fs-12 text-muted fw-semibold">Available for Payout</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Restaurant Bank Profile Summary -->
            ...
            <div class="row align-items-center g-3">
                <div class="col-lg-8">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h6 class="fw-bold text-dark mb-1">{{ $restaurant?->restaurant_name ?? 'Restaurant' }} - Settlement Account</h6>
                            <div class="fs-12 text-muted">
                                @if($restaurant?->account_number)
                                    Bank: <strong>{{ $restaurant->bank_name ?? 'Bank' }}</strong> • 
                                    A/C: <strong>••••{{ substr($restaurant->account_number, -4) }}</strong> • 
                                    IFSC: <strong>{{ $restaurant->ifsc_code ?? '-' }}</strong> • 
                                    PAN: <strong>{{ $restaurant->pan_number ?? '-' }}</strong>
                                @else
                                    <span class="text-warning"><i class="feather-alert-circle me-1"></i>No default settlement bank account set. You can provide it during withdrawal request.</span>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-4 text-lg-end">
                    <button type="button" class="btn btn-primary rounded-pill px-4 fw-semibold shadow-sm" data-bs-toggle="modal" data-bs-target="#requestWithdrawalModal">
                        <i class="feather-arrow-up-circle me-1"></i>Request Payout
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="card stretch stretch-full border-0 shadow-sm rounded-4 overflow-hidden bg-white">
        <div class="card-header bg-white border-bottom py-3 d-flex align-items-center justify-content-between">
            <h5 class="card-title mb-0 fw-bold fs-15 text-dark">
                <i class="feather-list me-2 text-primary"></i>Payout Request History
            </h5>
                                ...
                                <td>
                                    @if($w->status === 'approved')
                                        <span class="status-badge bg-soft-success text-success">
                                            <i class="feather-check-circle me-1"></i>Paid / Settled
                                        </span>
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-5">
                                    <div class="text-muted py-4">
                                        ...
                                        <h6 class="fw-bold text-dark mb-1">No Payout Requests</h6>
                                        <p class="fs-12 text-muted mb-0">Your submitted wallet withdrawal requests will be displayed here.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
