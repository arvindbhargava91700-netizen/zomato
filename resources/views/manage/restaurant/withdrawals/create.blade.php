...
@section('title', 'Request Payout - Restaurant Partner')
...
<div class="page-header d-flex align-items-center justify-content-between mb-4">
    <div class="page-header-left d-flex align-items-center">
        <div class="page-header-title">
            <h5 class="m-b-10 fw-bold">Request Restaurant Payout</h5>
        </div>
        <ul class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('restaurant.dashboard') }}">Home</a></li>
            <li class="breadcrumb-item"><a href="{{ route('restaurant.withdrawals.index') }}">Withdrawals</a></li>
            <li class="breadcrumb-item">New Payout</li>
        </ul>
    </div>
    <div class="page-header-right">
        <a href="{{ route('restaurant.withdrawals.index') }}" class="btn btn-sm btn-light border shadow-sm px-3">
            <i class="feather-arrow-left me-1"></i>Back to Payouts
        </a>
    </div>
</div>
<!-- [ page-header ] end -->
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white mb-4">
                <div class="card-header bg-white border-bottom py-3 px-4">
                    <h5 class="card-title mb-0 fw-bold fs-16 text-dark d-flex align-items-center gap-2">
                        <i class="feather-arrow-up-circle text-primary"></i>Withdraw Restaurant Earnings
                    </h5>
                </div>
                <div class="card-body p-4 p-md-5">
                    ...
                    <form id="restaurantWithdrawalForm" method="POST" action="{{ route('restaurant.withdrawals.store') }}">
                        @csrf

                        <!-- 1. Amount Section -->
                        <div class="mb-4">
                            <label class="form-label fs-13 fw-bold text-dark text-uppercase mb-2">
                                1. Enter Payout Amount <span class="text-danger">*</span>
                            </label>
                            <div class="input-group input-group-lg">
                                ...
                                <input type="number" step="0.01" min="10" max="{{ max(0, (float)$wallet->balance) }}" 
                                       id="withdrawAmountRes" name="amount" value="{{ old('amount') }}" 
                            ...
                            <div class="d-flex gap-2 mt-3 flex-wrap align-items-center">
                                ...
                                <span class="quick-chip" onclick="setAmount(5000)">₹5,000</span>
                                <span class="quick-chip" onclick="setAmount(10000)">₹10,000</span>
                                @if($wallet->balance > 0)
                        ...
                        <div class="mb-4">
                            <label class="form-label fs-13 fw-bold text-dark text-uppercase mb-2">
                                2. Select Transfer Method <span class="text-danger">*</span>
                            </label>
                            <div class="row g-3">
                                <div class="col-sm-6">
                                    <input type="radio" class="btn-check" name="payout_method" id="res_method_bank" value="bank_transfer" 
                                           {{ old('payout_method', 'bank_transfer') === 'bank_transfer' ? 'checked' : '' }} onchange="toggleMethod('bank')">
                                    <label class="method-card-label w-100 d-flex align-items-center gap-3" for="res_method_bank">
                                ...
                                <div class="col-sm-6">
                                    <input type="radio" class="btn-check" name="payout_method" id="res_method_upi" value="upi" 
                                           {{ old('payout_method') === 'upi' ? 'checked' : '' }} onchange="toggleMethod('upi')">
                                    <label class="method-card-label w-100 d-flex align-items-center gap-3" for="res_method_upi">
                                        ...
                                        <div>
                                            ...
                                            <small class="text-muted fs-11">VPA / Business UPI Handle</small>
                                        </div>
                                    </label>
                                </div>
                            </div>
                        ...
                        <div id="bankSection" style="{{ old('payout_method', 'bank_transfer') === 'bank_transfer' ? 'display:block;' : 'display:none;' }}">
                            ...
                            <div class="p-3 p-md-4 rounded-4 bg-light border mb-4">
                                <div class="mb-3">
                                    <label class="form-label fs-12 fw-bold text-muted text-uppercase mb-1">Beneficiary Name</label>
                                    <input type="text" name="holder_name" value="{{ old('holder_name', $restaurant?->holder_name ?? $owner->holder_name ?? $restaurant?->restaurant_name ?? $owner->name) }}" class="form-control" placeholder="Beneficiary name as per bank records">
                                </div>
                                <div class="row g-3 mb-3">
                                    <div class="col-sm-6">
                                        <label class="form-label fs-12 fw-bold text-muted text-uppercase mb-1">Bank Name</label>
                                        <input type="text" name="bank_name" value="{{ old('bank_name', $restaurant?->bank_name ?? $owner->bank_name) }}" class="form-control" placeholder="e.g. HDFC Bank">
                                    </div>
                                    <div class="col-sm-6">
                                        <label class="form-label fs-12 fw-bold text-muted text-uppercase mb-1">IFSC Code</label>
                                        <input type="text" name="ifsc_code" value="{{ old('ifsc_code', $restaurant?->ifsc_code ?? $owner->ifsc_code) }}" class="form-control text-uppercase" placeholder="HDFC0001234">
                                    </div>
                                </div>
                                <div>
                                    <label class="form-label fs-12 fw-bold text-muted text-uppercase mb-1">Bank Account Number</label>
                                    <input type="text" name="account_number" value="{{ old('account_number', $restaurant?->account_number ?? $owner->bank_account) }}" class="form-control" placeholder="Enter bank account number">
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="upiSection" style="{{ old('payout_method') === 'upi' ? 'display:block;' : 'display:none;' }}">
                            <label class="form-label fs-13 fw-bold text-dark text-uppercase mb-2">
                                3. UPI Settlement Handle
                            </label>
                            <div class="p-3 p-md-4 rounded-4 bg-light border mb-4">
                                <div>
                                    <label class="form-label fs-12 fw-bold text-muted text-uppercase mb-1">UPI ID / VPA <span class="text-danger">*</span></label>
                                    <input type="text" name="upi_id" value="{{ old('upi_id', $restaurant?->upi_id ?? $owner->upi_id) }}" class="form-control" placeholder="e.g. restaurant@hdfcbank">
                                    <small class="text-muted fs-11 mt-1 d-block">Make sure your business UPI ID is active to receive payouts.</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="notes" value="{{ old('notes') }}" class="form-control" placeholder="e.g. Food earnings payout request">
                        </div>

                        <!-- Action Buttons -->
                        <div class="d-flex gap-3 justify-content-end pt-3 border-top">
                            <a href="{{ route('restaurant.withdrawals.index') }}" class="btn btn-light border px-4 py-2 fw-semibold">
                                Cancel
                            </a>
                            <button type="submit" id="btnSubmitResWithdrawal" class="btn btn-primary px-5 py-2 fw-semibold shadow-sm position-relative" {{ $wallet->balance <= 0 ? 'disabled' : '' }}>
                                <span class="btn-text d-inline-flex align-items-center">
                                    <i class="feather-send me-2"></i>Confirm & Submit Payout
                                </span>
        ...
        <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm rounded-4 summary-box p-4 mb-4">
                ...
                <div class="d-flex justify-content-between pt-2 border-top mb-3">
                    <span class="fw-bold text-dark fs-14">Net Settlement:</span>
            ...
            <div class="card border-0 shadow-sm rounded-4 bg-white p-4">
                <h6 class="fw-bold text-dark mb-3 d-flex align-items-center gap-2">
                    <i class="feather-shield text-success"></i>Settlement Guidelines
                </h6>
                <ul class="list-unstyled mb-0 fs-12 text-muted d-flex flex-column gap-2">
                    <li class="d-flex align-items-start gap-2">
                        <i class="feather-check text-success mt-1"></i>
                        <span>Minimum payout request amount is <strong>{{ $currencySym }}10.00</strong>.</span>
                    </li>
                    <li class="d-flex align-items-start gap-2">
                        <i class="feather-check text-success mt-1"></i>
                        <span>Settlements are processed directly to the registered restaurant bank account.</span>
                    </li>
                    <li class="d-flex align-items-start gap-2">
                        <i class="feather-check text-success mt-1"></i>
...
<script>
    function setAmount(val) {
        document.getElementById('withdrawAmountRes').value = val;
        updateSummary(val);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.getElementById('restaurantWithdrawalForm');
        const btn = document.getElementById('btnSubmitResWithdrawal');
