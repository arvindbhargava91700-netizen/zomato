...
@section('title', 'Food Categories - Zomato Partner')
    ...
    <div class="page-header">
        <div class="page-header-left d-flex align-items-center">
            <div class="page-header-title">
                <h5 class="m-b-10">My Food Categories</h5>
            </div>
            <ul class="breadcrumb">
        ...
        <div class="page-header-right ms-auto d-flex align-items-center gap-2">
            <a href="{{ route('restaurant.food-categories.create') }}" class="btn btn-danger text-white fw-semibold" style="background-color: #cb202d; border: none;">
    ...
    <div class="main-content">
        ...
        <div class="card stretch stretch-full mb-4 border-0 shadow-sm rounded-3">
            <div class="card-body p-3">
                <form action="{{ route('restaurant.food-categories.index') }}" method="GET" class="row g-2 align-items-center">
                    <div class="col-md-5">
                    ...
                    <div class="col-md-4 d-flex gap-2">
                        ...
                        <a href="{{ route('restaurant.food-categories.index') }}" class="btn btn-light border text-secondary fw-semibold">Reset</a>
                        <a href="{{ route('restaurant.food-categories.index', ['trashed' => 1]) }}" class="btn btn-outline-danger fw-semibold" title="View Trashed Categories">
        ...
        <div class="card stretch stretch-full border-0 shadow-sm rounded-3">
            <div class="card-header border-bottom py-3">
                <h5 class="card-title mb-0 fw-bold">Categories for {{ $restaurant->restaurant_name }}</h5>
            </div>
                        ...
                        <tbody>
                            @forelse($categories as $category)
                                <tr>
                                    ...
                                    <td>
                                        @if(!$category->trashed())
                                            <form action="{{ route('restaurant.food-categories.toggle-status', $category->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="d-flex align-items-center justify-content-end gap-1">
                                            @if(!$category->trashed())
                                                <a href="{{ route('restaurant.food-categories.show', $category->id) }}" class="btn btn-sm btn-light border text-info p-2 rounded-2" data-bs-toggle="tooltip" title="View Details">
                                                    <i class="feather-eye"></i>
                                                </a>
                                                <a href="{{ route('restaurant.food-categories.edit', $category->id) }}" class="btn btn-sm btn-light border text-primary p-2 rounded-2" data-bs-toggle="tooltip" title="Edit">
                                                    <i class="feather-edit"></i>
                                                </a>
                                                <form action="{{ route('restaurant.food-categories.destroy', $category->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to soft delete this category?');">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @else
                                                <form action="{{ route('restaurant.food-categories.restore', $category->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    <button type="submit" class="btn btn-sm btn-success p-2 rounded-2" title="Restore">
                                                        <i class="feather-rotate-ccw"></i> Restore
                                                    </button>
                                                </form>
                                                <form action="{{ route('restaurant.food-categories.force-delete', $category->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to permanently delete this category?');">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-5 text-muted">
                                        ...
                                        No food categories found for your restaurant.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
