...
@section('title', 'Transaction Voucher #' . $transaction->transaction_number . ' - Delivery Partner')

@push('styles')
<style>
    /* -------------------------------------------------------------
       FINTECH TRANSACTION SLIP - PREMIUM STYLING
       ------------------------------------------------------------- */
    .slip-page-bg {
        padding: 24px 0 60px;
        min-height: 80vh;
    ...
    @media print {
        ...
        .page-header, .page-header-right, .nxl-navigation, .nxl-header, .nxl-footer, .no-print {
...
<div class="page-header d-flex align-items-center justify-content-between mb-4 no-print">
    <div class="page-header-left d-flex align-items-center">
        <div class="page-header-title">
            <h5 class="m-b-10 fw-bold">Official Transaction Slip</h5>
        </div>
        <ul class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('delivery-partner.dashboard') }}">Home</a></li>
            <li class="breadcrumb-item"><a href="{{ route('delivery-partner.transactions.index') }}">Transactions</a></li>
            <li class="breadcrumb-item">Slip #{{ $transaction->id }}</li>
        </ul>
    </div>
    <div class="page-header-right d-flex gap-2">
        <a href="{{ route('delivery-partner.transactions.index') }}" class="btn btn-sm btn-light border shadow-sm px-3">
            <i class="feather-arrow-left me-1"></i>All Transactions
        </a>
...
@php
    ...
    $companyTitle = $setting->company_name ?? 'FOOD DELIVERY';
    
    // Friendly source labels & icons
    $sourceMeta = [
        'delivery_partner_payout' => [
            'label' => 'Delivery Commission Payout',
            'desc'  => 'Order delivery earnings credited to wallet',
            'icon'  => 'feather-truck',
            'bg'    => 'bg-soft-success',
            'color' => 'text-success'
        ],
        'cod_cash_collected' => [
            'label' => 'COD Cash-in-Hand Collection',
        ...
        'manual' => [
            ...
            'desc'  => 'Manual credit/debit adjustment by admin',
...
<div class="main-content slip-page-bg">
    <div class="voucher-card-wrapper">
        <div class="voucher-paper">
            ...
            <!-- -------------------------------------------------------------
                 1. SLIP HERO HEADER (Vibrant Gradient Banner)
                 ------------------------------------------------------------- -->
            <div class="{{ $isCredit ? 'voucher-hero-credit' : 'voucher-hero-debit' }} text-center">
                ...
                <div class="d-flex align-items-center justify-content-between mb-4">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="text-start">
                            ...
                            <span class="fs-10 text-white-50 text-uppercase fw-semibold">Delivery Partner Network</span>
                        </div>
                    </div>
                ...
                <div class="d-inline-flex align-items-center gap-2 px-3 py-1 rounded-pill my-1" 
                    ...
                    <span class="text-white fs-12 fw-semibold">
                        {{ $isCredit ? 'Credited to Partner Balance' : 'Debited from Partner Balance' }}
                    </span>
                </div>
            ...
            <div class="p-4 p-md-5 pt-3 pb-4">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-sm-6">
                        <div class="data-pill-card d-flex align-items-center gap-3">
                            ...
                            <div>
                                <span class="fs-11 text-muted text-uppercase fw-bold d-block">Transaction Type</span>
                    ...
                    <div class="col-sm-6">
                        <div class="data-pill-card d-flex align-items-center gap-3">
                            <div class="item-icon-wrap bg-soft-secondary text-secondary">
                                <i class="feather-user"></i>
                            </div>
                            <div>
                                <span class="fs-11 text-muted text-uppercase fw-bold d-block">Beneficiary Courier</span>
                                <span class="fs-13 fw-bold text-dark">{{ auth()->user()->name }}</span>
                                <span class="fs-11 text-muted d-block">ID: #{{ auth()->id() }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="data-pill-card d-flex align-items-center gap-3">
                            <div class="item-icon-wrap bg-soft-success text-success">
                                <i class="feather-phone"></i>
                            </div>
                            <div>
                                <span class="fs-11 text-muted text-uppercase fw-bold d-block">Courier Mobile</span>
                                <span class="fs-13 fw-bold text-dark">{{ auth()->user()->phone ?? 'N/A' }}</span>
                                <span class="fs-11 text-muted d-block">{{ auth()->user()->email ?? '' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-header bg-light py-3 px-3 border-bottom d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center gap-2">
                                <span class="badge bg-primary text-white px-2 py-1 fs-12 rounded">
                                    <i class="feather-shopping-bag me-1"></i>Order #{{ $order->id }}
                                </span>
                                <span class="fs-12 text-muted fw-semibold">Associated Delivery Details</span>
                            </div>
                            <a href="{{ route('delivery-partner.orders.show', $order->id) }}" class="btn btn-xs btn-outline-primary rounded-pill px-3 fw-semibold">
                                <i class="feather-external-link me-1"></i>View Full Order
                            </a>
                        </div>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-sm-6">
                                    <div class="d-flex align-items-start gap-2">
                                        ...
                                        <div>
                                            <span class="text-muted fs-11 text-uppercase fw-bold d-block">Pickup Restaurant</span>
                                ...
                                <div class="col-sm-6">
                                    <div class="d-flex align-items-start gap-2">
                                        ...
                                        <div>
                                            <span class="text-muted fs-11 text-uppercase fw-bold d-block">Drop Customer</span>
                                            <span class="fw-bold text-dark fs-14">{{ $order->user?->name ?? 'Customer' }}</span>
                                            <span class="text-muted fs-11 d-block">{{ $order->address?->address ?? 'Customer Location' }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-12 pt-3 border-top">
                                    <div class="row g-2 text-center">
                                        ...
                                        <div class="col-4">
                                            <span class="text-muted fs-11 text-uppercase fw-bold d-block">Earned Delivery Fee</span>
                                            <span class="fw-bold text-success fs-13">+{{ $currencySym }}{{ number_format($transaction->amount, 2) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="p-3 bg-light rounded-4 border text-center position-relative mb-2">
                    <div class="row align-items-center g-3">
                        <!-- Verification Seal & QR Code simulation -->
                ...
                <div class="text-center mt-3">
                    <p class="fs-11 text-muted mb-0">
                        This is a computer-generated digital transaction receipt issued for delivery partner ledger reconciliation. No physical signature required.
                    </p>
                </div>
            </div>
        ...
        <div class="d-flex flex-wrap gap-2 justify-content-center mt-4 no-print">
            <button type="button" onclick="window.print()" class="btn btn-primary px-4 py-2 rounded-pill shadow-sm fw-semibold">
                <i class="feather-printer me-2"></i>Print Official Voucher
            </button>
            <button type="button" class="btn btn-outline-dark px-4 py-2 rounded-pill fw-semibold" 
                    onclick="if(navigator.share){navigator.share({title:'Transaction Slip {{ $transaction->transaction_number }}', text:'Delivery Partner Voucher for {{ $currencySym }}{{ number_format($transaction->amount, 2) }}', url:window.location.href});}else{navigator.clipboard.writeText(window.location.href); alert('Voucher link copied to clipboard!');}">
                <i class="feather-share-2 me-2"></i>Share Slip Link
            </button>
            <a href="{{ route('delivery-partner.transactions.index') }}" class="btn btn-light border px-4 py-2 rounded-pill text-muted fw-semibold">
