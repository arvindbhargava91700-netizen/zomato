    ...
    <div class="main-content">
        <div class="card stretch stretch-full border-0 shadow-sm rounded-3">
            <div class="card-body p-4">
                <form action="{{ route('admin.food-variants.update', $foodVariant->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="food_id" id="food_id" class="form-select @error('food_id') is-invalid @enderror" required>
                                @foreach($foods as $food)
                                    <option value="{{ $food->id }}" {{ old('food_id', $foodVariant->food_id) == $food->id ? 'selected' : '' }}>{{ $food->name }} ({{ $food->restaurant->restaurant_name ?? '' }})</option>
                                @endforeach
                            </select>
