...
@section('title', 'Edit Food Item - Admin Dashboard')
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="restaurant_id" id="restaurant_id" class="form-select @error('restaurant_id') is-invalid @enderror" required>
                                @foreach($restaurants as $restaurant)
                                    <option value="{{ $restaurant->id }}" {{ old('restaurant_id', $food->restaurant_id) == $restaurant->id ? 'selected' : '' }}>{{ $restaurant->restaurant_name }}</option>
                                @endforeach
                            </select>
